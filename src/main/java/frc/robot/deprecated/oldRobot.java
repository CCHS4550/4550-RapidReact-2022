...
public class oldRobot extends TimedRobot implements ControlMap {
...
//stage deez
  ...
  public void teleopPeriodic() {
    ...
    oldChassis.toggleFast(OI.button(0, Y_BUTTON));
    ...
    oldChassis.axisDrive(velocity, rJoystick, 1);
    ...
    oldTedBallin.runIndexer(OI.button(1, LB_BUTTON), OI.axis(1, LT) >= 0.1, false, 0.5);

    //RB for fast shoot, RT for reverse
    oldTedBallin.runShooter(OI.axis(1, RT) >= 0.1, OI.button(1, RB_BUTTON), false, shootSpeed.value(), 4);

    //A for in, B for out
    //Intake.run(OI.axis(1, LT) >= 0.1, OI.button(1, LB_BUTTON), false, -1);

    //Climbing Arms Toggle (Y)
    oldArms.toggleArms(OI.button(1, Y_BUTTON));

    //Intake Arms Toggle (X)
    //Intake.toggleIntake(OI.button(1, X_BUTTON));

    //Fast Mode Toggle (A)
    oldChassis.toggleFastMode(OI.button(0, A_BUTTON), OI.joystickArray[0]);
